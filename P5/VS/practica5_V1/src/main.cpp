...
#include "sensorLM35.hpp"//cabecera hehca para la practica 5, solo retorna temperatura
LM35 myLM35 = {4, 5};//el lm35 solo necesita un pin ADC, es el 4; el 5 lo uso como GND siempre es digital en LOW
#include "ultrasonico.hpp"
senUltra miUltrasonico = {8, 18};//echo y trig
#include "miPuenteH.hpp"
puenteH myMotors = {12, 11, 10, 9};//Los 4 pines para las entradas del puente H
bool stateMotors = false;
unsigned long timeMotorsStart = 0, timeMotorsOn = 0;
int comandoAux;
int timeON[4] = {250, 250, 100, 100};//Tiempo de activacion para las 4 direcciones, milisegundos
  ...
  if(type == WS_EVT_DATA){
    ...
    if(len > 0){//El mensaje existe
      ...
      if(!error){
        ...
        }else if(tipo == "getTemperatura"){
          float temperatura = myLM35.getCentigrados();
          Serial.print("Temperatura medida: ");
          Serial.print(temperatura);
          Serial.println("C°");
          //Crear objeto JSON para enviar las mediciones
          DynamicJsonDocument doc(256);
          doc["type"] = "medicionTemperatura";
...
void busquedaTask(void *param){
  while(true){
    if(iniciarBusqueda && clientBusqueda != nullptr){
      ...
      respuesta["type"] = "medicionDistancia";
      respuesta["distancia"] = busDis;
      String jsonString;
